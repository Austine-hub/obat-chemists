// ===============================================================
// 💊 HTNDetails.tsx — Detailed View for Hypertension Products
// Mirrors structure & logic of RespiratoryDetails.tsx
// ===============================================================
...
import { htnProducts, type Product } from "../data/HTNData";
import styles from "./RespiratoryDetails.module.css";

const HTNDetails: React.FC = () => {
  ...
  // ✅ Fetch product by ID with delay for smooth skeleton effect
  useEffect(() => {
    setIsLoading(true);
    const timer = setTimeout(() => {
      const foundProduct = htnProducts.find(
  ...
  const handleSimilarProductClick = useCallback((productId: number) => {
    ...
    navigate(`/htn-product/${productId}`);
  ...
  return (
    <section className={styles.container}>
      ...
      <nav className={styles.breadcrumb} aria-label="Breadcrumb">
        ...
        <button onClick={() => handleBreadcrumbClick("/htn")} className={styles.breadcrumbLink}>
          Hypertension
        </button>
        <span className={styles.breadcrumbSeparator}>/</span>
      ...
      <article className={styles.detailsSection}>
        <div className={styles.imageWrapper}>
          ...
          {!isInStock && <div className={styles.outOfStockBadge}>Out of Stock</div>}
        </div>
            ...
            <div className={styles.tabContent}>
              {activeTab === "features" && (
                <ul className={styles.featuresList}>
                  ...
                  <li>Formulated by trusted global pharmaceutical brands</li>
              ...
              {activeTab === "description" && (
                <div className={styles.description}>
                  <p>
                    This antihypertensive medication is formulated to effectively lower and maintain blood
                    pressure at safe levels. It helps reduce cardiovascular risk and prevents complications
                    such as stroke and heart failure.
                  </p>
                  <p>
                    Manufactured under stringent GMP standards, ensuring quality, safety, and consistent efficacy.
                  </p>
                </div>
              )}
              ...
              {activeTab === "usage" && (
                <div className={styles.usage}>
                  <p><strong>Dosage:</strong> As directed by your healthcare provider</p>
                  <p><strong>Administration:</strong> Take at the same time daily with or without food</p>
                  <p><strong>Storage:</strong> Store in a cool, dry place away from direct sunlight</p>
                  <p className={styles.warning}>⚠️ Do not discontinue without consulting your doctor</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className={styles.actionsSection}>
            <div className={styles.quantityControl}>
              ...
              <div className={styles.quantityButtons}>
                <button onClick={decrementQty} className={styles.qtyBtn} aria-label="Decrease quantity" disabled={!isInStock}>−</button>
                <input
                  id="quantity"
                  type="number"
                  value={quantity}
                ...
                />
                <button onClick={incrementQty} className={styles.qtyBtn} aria-label="Increase quantity" disabled={!isInStock}>+</button>
              </div>
            </div>
            ...
            {similarProducts.map((item) => (
              ...
              >
                <div className={styles.similarImageWrapper}>
                  <img src={item.image} alt={item.name} className={styles.similarImage} loading="lazy" />
                </div>
                <div className={styles.similarInfo}>
                  <h3 className={styles.similarName}>{item.name}</h3>
                  <p className={styles.similarPrice}>KES {item.price.toLocaleString()}</p>
